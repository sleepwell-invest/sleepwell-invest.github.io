...
function stageTargetFor(assets){
  if(assets<500000)return 1000000;
  if(assets<2000000)return 3000000;
  if(assets<4000000)return 5000000;
  if(assets<8000000)return 10000000;
  let target=Math.ceil(assets/5000000)*5000000;
  if(target-assets<500000)target+=5000000;
  return target;
}
function calculate(){
  ...
  const stageCapital=stageTargetFor(assets);
  const stageShortfall=Math.max(0,stageCapital-assets);
  const stageMonthly=factor>0?stageShortfall/factor:stageShortfall;
  ...
  document.querySelector('#stageExplain').textContent=stageShortfall===0?'依目前假設，既有資產成長後可望達成這個階段。':'先完成這個資產里程碑，再逐步往下一階段前進。';
  document.querySelector('#statusMessage').textContent=stageShortfall===0?'依目前假設，暫時不需為這個階段額外增加投入。':'以 '+money(stageCapital)+' 為下一階段目標，不用一開始就背負全部退休金。';
  results.hidden=false;
  results.scrollIntoView({behavior:'smooth',block:'start'});
  window.lastResult={currentAge,retireAge,planAge,preYears,retirementYears,expense,income,assets,firstGap,capitalNeed,futureAssets,shortfall,monthlySaving,stageMonthly,stageIncome,stageCapital,stageShortfall,inflation,preReturn,postReturn};
}
